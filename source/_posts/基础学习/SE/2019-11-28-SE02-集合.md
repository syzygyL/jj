...
# 容器 
  基本的类型是Lst、Set、 Queue 和 Map。这些对象类型也称为集合类，Java 类库中使用了 Collection 这个名字来指代该类库的—个特殊子集（Lst、Set、 Queue）
...
![](http://q253h0pkr.bkt.clouddn.com/FkrZg1PdlpLp9_7UiVohH7fq7Tgy)
...
所有的新实现都有 fail-fast(快速报错) 的迭代器，它检测无效的并发修改，并且快速而干净地失败

#### Collection

  Collection是所有单列集合（List、Ser、Queue）的父接口
...
#### foreach
  增强型 for 循环，用于遍历数组、集合，底层实现为迭代器
  
  格式：for( 元素数据类型 变量名 : 数组名\集合名 ){ ... }

### List
  java.util.List 接口继承自 Collection 接口，将实现了 `List` 接口的类统称为 List 集合。主要的三个实现类：1. ArrayList 2. LinkedList 3. Vector（不常用）
...
```java
  public static void main(String[] args) {
        ...
        // list1.add("1");                 // 抛出 NullPointerException
        List list2 = new ArrayList<>(); // 创建一个空集合(size()等于零)
        println(list1);
        println(list2);
        /** Output:
         * null
...
#### ArrayList
  **特点：有索引，查询块，增删慢**
  底层为 Object[] ，初始容量为10，超过则按 50% 增长。
#### LinkedList
  **特点：无索引，查询慢，增删快**
  底层为双向链表( JDK1.6 之前为循环链表，JDK1.7 取消了循环)
#### Vector（不常用，淘汰）
  **特点：方法同步的( Synchronized )，线程安全的**
  底层为 Object[]，初始容量为10，超过则翻倍增长。（同步影响其性能）
#### RandomAccess
java.util.RandomAccess 接口 ，内部无任何方法（标志接口）
...
#### HashSet
  **特点：无索引，元素唯一，存取无序**
  ...
  允许空键 null ，基于 `HashMap` 实现，底层通过 HashMap 来保存元素来修改集合元素，除了 clone()、writeObject()、readObject() 是 HashSet 自己不得不实现之外，其他方法都是直接调用 HashMap 中的方法。

  **通过重写存储对象中的 hashCode 和 equals 方法保证元素唯一性**
  
#### LinkedHashSet
  **特点：无索引，元素唯一，存取有序** 
  继承自 HashSet  ，由链表保证存取有序
...
### HashMap
  HashMap 是 Map 基于哈希表的实现，插入和查询的开销是固定的，可以通过构造器设置容量和负载因子，以调整 HashMap 性能
...
## Set（Key 和 Value 相同）如何保证元素不重复，跟 Map 大同小异
